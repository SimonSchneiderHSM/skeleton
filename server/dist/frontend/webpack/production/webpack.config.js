...
var appConfig = frontendConfig.appConfig;
var WebpackIsomorphicToolsConf = require('@process-engine-js/frontend/dist/helpers/devtools/webpack/webpack-isomorphic-tools');
var isoTools = WebpackIsomorphicToolsConf(appConfig);

exports = module.exports = function (config, webpackIsomorphicTools) {
  ...
  if (fs.existsSync(path.resolve(config.schemaHelperPath, 'graphql-schema.json'))) {
    babelLoaderQuery.plugins = [path.resolve(config.webpackConfigPath, 'babelRelayPlugin.js')];
  }

  babelLoaderQuery.only = ["./src/frontend/mainApp.js", "./src/frontend/index.js", "./src/frontend/components/**/*.js", "./src/frontend/containers/**/*.js", "./src/frontend/redux/**/*.js", "./src/frontend/relay/**/*.js", "./src/frontend/theme/**/*.js", "./src/frontend/helpers/**/*.js"];

  babelLoaderQuery.presets = ["react", ["es2015", { modules: false }], "stage-0"];
  babelLoaderQuery.plugins = babelLoaderQuery.plugins.concat(["add-module-exports", "transform-decorators-legacy", "transform-react-display-name", "syntax-async-functions", "transform-regenerator", "material-ui", "lodash", ["transform-runtime", {
    "polyfill": false,
    "regenerator": true
  }]]);
  ...
  return {
    ...
    entry: {
      'main': ['bootstrap-loader', 'font-awesome-loader!' + path.resolve(config.appConfig.app.themePath, config.appConfig.app.theme, 'font-awesome.config.js'), '' + path.resolve(config.appConfig.app.startScript)]
    },
    output: {
      path: assetsPath,
    ...
    plugins: [new HappyPack({
      ...
      failOnError: true
    }), new CleanPlugin([assetsPath], { root: appRoot }), new ExtractTextPlugin({ filename: '[name]-[chunkhash].css', disable: false, allChunks: true }), new Webpack.DefinePlugin({
      'process.env': {
        NODE_ENV: '"production"'
      },
